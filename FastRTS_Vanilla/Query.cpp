...
void Query::ComputeThresholdOnSite() {
    double rest = this->threshold - this->evlCnt;
...
int Query::ReceiveNotification(void *quadTree_log, double ptCntIncrease) {
...
double Query::CollectPt(void *quadTree_log) {
    auto* log=(QuadTreeLog*) quadTree_log;
	double e=RHO*(range[1]-range[0]);
	return log->getRoot()->CanonicalNodeCounter(*this,quadTree_log,e);
}


double Query::CollectInfoFromSites(void *quadTree_log) {
    auto* log=(QuadTreeLog*) quadTree_log;
    double ptCnt = 0;
	nodeIndexList.clear();
...
void Query::EnterNextEpoch(void *quadTree_log,bool needFind) {
	...
	if (this->threshold<EXA_THE*2){
		this->epoch_n=EPO-1;
	}
	nodeIndexList.clear();
	int length=range[1]-range[0];
	double e1=rho_epoch[epoch_n]*length;
	double e2=RHO*length;
	double pre=this->evlCnt;
	int p_n=this->siteNum;
	this->evlCnt=log->getRoot()->FindCanonicalNode_Epoch(*this,log,Query::nodeIndexList,e1,e2);
...
void Query::GlobalRebuild(void *quadTree_log) {
    this->evlCnt=CollectPt(quadTree_log)-p_cnt;
